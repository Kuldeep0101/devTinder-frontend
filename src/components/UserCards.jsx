        ...
        <div className="card bg-white w-96 rounded-2xl shadow-xl border border-gray-200">
          ...
          <div className="card-body p-6 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-2 mb-2">
              {skills && skills[0] && (
                <div className="badge badge-outline bg-blue-100 text-blue-800 border-blue-200 p-3 rounded-full font-medium">
                  {skills[0]}
                </div>
              )}
              {skills && skills[1] && (
                <div className="badge badge-outline bg-green-100 text-green-800 border-green-200 p-3 rounded-full font-medium">
                  {skills[1]}
                </div>
              )}
              {skills && skills[2] && (
                <div className="badge badge-outline bg-purple-100 text-purple-800 border-purple-200 p-3 rounded-full font-medium">
                  {skills[2]}
                </div>
              )}
              {skills && skills[3] && (
                <div className="badge badge-outline bg-yellow-100 text-yellow-800 border-yellow-200 p-3 rounded-full font-medium">
                  {skills[3]}
                </div>
              )}
              {skills && skills[4] && (
                <div className="badge badge-outline bg-red-100 text-red-800 border-red-200 p-3 rounded-full font-medium">
                  {skills[4]}
                </div>
              )}
            </div>
          ...
          <div className="flex justify-evenly p-6 border-t border-gray-200">
            <button
              className="btn btn-outline btn-success flex-1 mx-2 rounded-full font-semibold transition-all duration-300 hover:scale-105"
              onClick={() => handleSendRequest("interested", _id)}
            >
              Interested
            </button>
            <button
              className="btn btn-outline btn-warning flex-1 mx-2 rounded-full font-semibold transition-all duration-300 hover:scale-105"
